...
void vm_gettabup(lua_State* L, int b, TValue *rkc, TValue *ra)
{
  ...
  if (luaV_fastget(L, upval, rkc, aux, luaH_get)) {
    setobj2s(L, ra, aux);
  }
  else {
    luaV_finishget(L, upval, rkc, ra, aux);
  }
}
...
void vm_settabup(lua_State* L, int a, TValue *rkb, TValue *rkc)
{
  LClosure *cl = clLvalue(L->ci->func);
  const TValue *slot;
  TValue *upval = cl->upvals[a]->v;

  if (!luaV_fastset(L, upval, rkb, slot, luaH_get, rkc)) {
    luaV_finishset(L, upval, rkb, rkc, slot);
  }
}
...
void vm_gettable(lua_State *L, const TValue *t, TValue *key, StkId val)
{
  ...
  if (luaV_fastget(L, t, key, aux, luaH_get)) {
    setobj2s(L, val, aux);
  }
  else {
    luaV_finishget(L, t, key, val, aux);
  }
}
...
void vm_closure(lua_State* L, TValue *ra, CallInfo *ci, int bx)
{
  ...
  luaC_condGC(L,L->top = ra+1, L->top = ci->top);
  luai_threadyield(L);
}
...
int vm_return(lua_State* L, TValue *ra, CallInfo *ci, int b)
{
  ...
  nb = luaD_poscall(L, ci, ra, (b != 0 ? b - 1 : cast_int(L->top - ra)));
...
{
  setobjs2s(L, ra, rb);
  luaC_condGC(L,L->top = (ra >= rb ? ra + 1 : rb), L->top = ci->top);
  luai_threadyield(L);
...
void vm_vararg(lua_State* L, CallInfo *ci, int a, int b)
{
  ...
  for (; j < nb; j++)  /* complete required results with nil */
    setnilvalue(ra + j);
}
